...
public class StatusBarUtil {
    ...
    public static void setTranslucentForWebview(Activity activity, @IntRange(from = 0, to = 255) int statusBarAlpha,int color,
                                                  View needOffsetView) {
        if (Build.VERSION.SDK_INT < Build.VERSION_CODES.KITKAT) {
            return;
        }
...
//            fakeTranslucentView.getBackground().setAlpha(statusBarAlpha);
//        } else {
...
//        }
        ...
        if (needOffsetView != null) {
            needOffsetView.getBackground().setAlpha(statusBarAlpha);
            Object haveSetOffset = needOffsetView.getTag(TAG_KEY_HAVE_SET_OFFSET);
            if (haveSetOffset != null && (Boolean) haveSetOffset) {
                return;
            }
